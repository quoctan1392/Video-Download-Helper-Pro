...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message && message.action === 'videoDetected') {
      ...
      if (message.video) {
        ...
        refreshTimeout = setTimeout(() => {
          console.log('[Popup] New video detected (no video object), refreshing list...');
          loadVideos();
        }, 1000);
      }
    ...
    if (message && message.action === 'log') {
      ...
      if (message.log) {
        ...
        addLog(message.level || 'info', message.source || 'Unknown', message.message || '', message.data);
      }
    }
  ...
  try {
    ...
    const testResponse = await chrome.tabs.sendMessage(currentTabId, { action: 'ping' }).catch(() => null);
  ...
  setInterval(() => {
    ...
    if (hasActiveDownloads) {
      console.log('[Popup] Skipping periodic refresh - active downloads in progress');
      return;
    }
  ...
  document.getElementById('errorInfoBtn').addEventListener('click', showErrorDetails);
  ...
  document.getElementById('copyErrorBtn').addEventListener('click', copyErrorToClipboard);
...
function updateVideoProgress(data) {
  ...
  const videoItem = document.querySelector(`.video-item[data-url="${CSS.escape(videoUrl)}"]`);
  if (!videoItem) {
  ...
  if (data.show) {
    ...
    if (data.paused === true && displayPercent === 0 && currentState.percent > 0) {
    ...
    downloadStates.set(videoUrl, {
      ...
      paused: data.paused !== undefined ? (data.paused === true) : (currentState.paused || false)
    });
    // Save to storage
    saveDownloadStates();
    ...
    if (data.percent >= 100) {
      setTimeout(() => {
        ...
        if (downloadBtn) {
          ...
          const isStreaming = video?.type === 'dash' || video?.type === 'hls' || video?.type === 'mpd';
        ...
        if (video && data.downloadId) {
          ...
          if (videoHeader && !videoHeader.querySelector('.downloaded')) {
            ...
            videoHeader.insertBefore(badge, videoHeader.querySelector('.video-duration'));
          }
          
          const videoActions = videoItem.querySelector('.video-actions');
          if (videoActions && !videoActions.querySelector('.folder-btn') && data.downloadId) {
            ...
            `;
            folderBtn.addEventListener('click', () => openDownloadFolder(data.downloadId));
            videoActions.insertBefore(folderBtn, videoActions.querySelector('.delete-btn'));
          }
        }
      }, 2000);
    }
...
async function copyLogToClipboard() {
  ...
  setTimeout(() => {
    updateStatus(`Tìm thấy ${currentVideos.length} video`, currentVideos.length > 0 ? '✅' : '❌');
  }, 2000);
}
...
function refreshLog() {
  ...
  if (logEntries.length === 0) {
    logConsole.innerHTML = '<div style="color: #858585; text-align: center; padding: 20px;">No logs</div>';
    return;
  }
  
  logConsole.innerHTML = logEntries.map(entry => {
...
async function saveVideosToStorage() {
  ...
  try {
    ...
    console.log('[Popup] Saved', currentVideos.length, 'videos to storage for tab', currentTabId);
  } catch (error) {
...
async function loadPersistedVideos() {
  ...
  try {
    ...
    const result = await chrome.storage.local.get(key);
      ...
      console.log('[Popup] Loaded', currentVideos.length, 'persisted videos from storage');
      displayVideos();
...
async function loadPersistedData() {
  ...
  try {
    ...
    const result = await chrome.storage.local.get(key);
    if (result[key] && Array.isArray(result[key])) {
      console.log('[Popup] Loaded', result[key].length, 'persisted videos for merging');
      return result[key];
    }
  } catch (error) {
...
function restoreDownloadProgress(videoUrl, state) {
  ...
  if (progressPercent) progressPercent.textContent = `${Math.round(state.percent || 0)}%`;
...
async function loadVideos() {
  ...
  console.log('[Popup] currentTabId:', currentTabId, 'typeof:', typeof currentTabId);
  ...
  try {
    ...
    if (allResponse && allResponse.videos && Array.isArray(allResponse.videos) && allResponse.videos.length > 0) {
      console.log('[Popup] SUCCESS: Found ALL videos:', allResponse.videos.length);
      ...
      console.log('[Popup] currentVideos merged with persisted data:', currentVideos);
    ...
    if (debugResponse && debugResponse.videos && Array.isArray(debugResponse.videos) && debugResponse.videos.length > 0) {
      console.log('[Popup] SUCCESS: Found debug videos:', debugResponse.videos.length);
    ...
    console.log('[Popup] Fallback Method: Requesting videos for current tab:', currentTabId);
    ...
    if (tabResponse && tabResponse.videos && Array.isArray(tabResponse.videos) && tabResponse.videos.length > 0) {
      console.log('[Popup] SUCCESS: Found tab videos:', tabResponse.videos.length);
...
async function displayVideos() {
  ...
  console.log('[Popup] Array.isArray(currentVideos):', Array.isArray(currentVideos));
  ...
  videoList.innerHTML = Array.from(uniqueVideos.values()).map((video, index) => 
    generateVideoItemHTML(video, index)
  ).join('');
...
function attachVideoEventListenersForItem(videoItem) {
  ...
  folderBtns.forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        addLog('info', 'Popup', `Opened file location for download ID: ${downloadId}`);
      } catch (error) {
        ...
        try {
          await chrome.downloads.search({ id: downloadId }).then(downloads => {
  ...
  downloadBtns.forEach((btn, btnIndex) => {
    btn.addEventListener('click', (e) => {
      ...
      const qualitySelect = document.querySelector(`.quality-select[data-index="${index}"]`);
      let quality = null;
  ...
  deleteBtns.forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      try {
        await chrome.runtime.sendMessage({ action: 'removeVideo', tabId: currentTabId, url });
        addLog('success', 'Popup', `Requested background to remove video: ${url}`);
      } catch (err) {
...
async function downloadVideoWithQuality(video, quality) {
  addLog('info', 'Popup', `downloadVideoWithQuality: quality=${quality ? quality.label : 'default'}`);
  
  try {
    const isStreaming = video.type === 'dash' || video.type === 'hls' || video.type === 'mpd';
    addLog('info', 'Popup', `Is streaming: ${isStreaming}, type: ${video.type}`);
    ...
    if (response.success) {
      ...
      addLog('success', 'Popup', `Download success! Segments: ${result.segmentCount || 'N/A'}`);
      ...
      if (result.segmentCount) {
        ...
        if (result.skipDownload) {
          ...
          addLog('warn', 'Popup', `FFmpeg required (no download): ${result.ffmpegCommand}`);
        } else {
          // Other streams - manifest downloaded
          updateStatus('FFmpeg required to convert the downloaded file', '⚠️');
          addLog('warn', 'Popup', `Manifest downloaded, FFmpeg required: ${result.ffmpegCommand}`);
        }
...
function truncateUrl(url, maxLength) {
  ...
  try {
    ...
    const truncated = domain + path.substring(0, maxLength - domain.length - 3) + '...';
    return truncated;
  } catch {
...
function getErrorSuggestions(error) {
  ...
  if (error.type === 'ffmpeg_required' && error.ffmpegCommand) {
    suggestions.push('🎬 Download and install FFmpeg from <a href="https://ffmpeg.org/download.html" target="_blank">ffmpeg.org</a>');
    suggestions.push('💻 Open Terminal/Command Prompt and run the FFmpeg command above');
    suggestions.push('📁 The resulting MP4 will be created in the current folder');
    return suggestions;
  }
  
  if (message.includes('cors') || message.includes('cross-origin')) {
    suggestions.push('Video blocked by CORS policy. Try downloading the manifest and use FFmpeg.');
    suggestions.push('Example: <code>ffmpeg -i manifest.m3u8 -c copy output.mp4</code>');
  }
  
  if (message.includes('manifest') || message.includes('fetch')) {
    suggestions.push('Check your internet connection.');
    suggestions.push('The video may require authentication. Try downloading directly from the site.');
  }
  
  if (message.includes('segments')) {
    suggestions.push('Video may be too large or segments are failing. Try using FFmpeg.');
    suggestions.push('Open Developer Console (F12) for more details.');
  }
  
  if (error.video?.type === 'dash' || error.video?.type === 'mpd') {
    suggestions.push('DASH stream: use FFmpeg: <code>ffmpeg -i video.mpd -c copy output.mp4</code>');
  }
  
  if (error.video?.type === 'hls') {
    suggestions.push('HLS stream: use FFmpeg: <code>ffmpeg -i video.m3u8 -c copy output.mp4</code>');
  }
...
function copyErrorToClipboard() {
  ...
  navigator.clipboard.writeText(errorText).then(() => {
    ...
    btn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><polyline points="20 6 9 17 4 12"/></svg> Copied!';
    setTimeout(() => {
