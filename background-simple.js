...
function isVideoUrl(url) {
  return url.includes('.mpd') || url.includes('.m3u8') || url.includes('video') || url.includes('.mp4');
}
...
class IntegratedStreamProcessor {
  ...
  pauseDownload(videoUrl) {
    console.log('[IntegratedStreamProcessor] pauseDownload() called for:', videoUrl);
    ...
    if (state) {
      ...
      if (state.abortController) {
        console.log('[IntegratedStreamProcessor] 🛑 ABORTING current segment download');
      ...
      const percent = completedSegments > 0 ? (completedSegments / totalSegments) * 100 : 0;
      ...
      console.warn('[IntegratedStreamProcessor] No download state found for:', videoUrl);
    }
  }
  ...
  resumeDownload(videoUrl) {
    ...
    if (state) {
      ...
      this.downloadStates.set(videoUrl, state);
      console.log('[IntegratedStreamProcessor] Download resumed from segment', state.currentSegment);
      
      // Use chunks.length (completed segments) for accurate progress
      const completedSegments = state.chunks?.length || 0;
      const percent = completedSegments > 0 ? (completedSegments / state.totalSegments) * 100 : 0;
  ...
  cancelDownload(videoUrl) {
    ...
    this.sendProgressUpdate(0, '❌ Download canceled', 'Canceled', '', videoUrl);
    setTimeout(() => {
  ...
  async handleStreamingProtocol(video, options) {
    ...
    console.log('[IntegratedStreamProcessor] Handling streaming protocol:', type);
    ...
    try {
      ...
      console.log(`[IntegratedStreamProcessor] Manifest fetched: ${manifestContent.length} characters`);
      
      // Parse manifest to get segments
      const segments = await this.extractSegments(manifestContent, url, type);
      
      console.log(`[IntegratedStreamProcessor] Extracted ${segments.length} segments`);
      
      if (!segments || (segments.length === 0 && !segments.separateStreams)) {
        console.warn('[IntegratedStreamProcessor] No segments found, using fallback');
        return await this.downloadManifestFallback(video, options);
      }
      ...
      console.error('[IntegratedStreamProcessor] Streaming protocol error:', error);
      return await this.downloadManifestFallback(video, options);
    }
  }
  ...
  async extractSegments(manifest, baseUrl, type) {
    ...
    if (type === 'hls') {
      ...
      if (isMasterPlaylist && bestVariantUrl) {
        console.log(`[IntegratedStreamProcessor] Fetching best variant: ${bestVariantUrl}`);
      ...
      if (isCloudflareStream) {
        ...
        const audioAdaptationSet = manifest.match(/<AdaptationSet[^>]*mimeType="audio[^"]*"[^>]*>[\s\S]*?<\/AdaptationSet>/i);
        
        console.log(`[IntegratedStreamProcessor] Found video: ${!!videoAdaptationSet}, audio: ${!!audioAdaptationSet}`);
        ...
        if (videoAdaptationSet) {
          const videoResult = this.parseAdaptationSet(videoAdaptationSet[0], baseUrl, duration, 'video');
          if (videoResult.segments.length > 0) {
            videoSegments.push(...videoResult.segments);
            console.log(`[IntegratedStreamProcessor] Video: ${videoSegments.length} segments, quality: ${videoResult.quality}`);
          }
        }
        
        // Process AUDIO AdaptationSet  
        if (audioAdaptationSet) {
          const audioResult = this.parseAdaptationSet(audioAdaptationSet[0], baseUrl, duration, 'audio');
          if (audioResult.segments.length > 0) {
            audioSegments.push(...audioResult.segments);
            console.log(`[IntegratedStreamProcessor] Audio: ${audioSegments.length} segments, quality: ${audioResult.quality}`);
          }
        }
        
        // Return combined segments or separate streams
        if (videoSegments.length > 0 && audioSegments.length > 0) {
          console.log('[IntegratedStreamProcessor] Found both video and audio - will download separately');
          return {
            video: videoSegments,
            audio: audioSegments,
  ...
  parseAdaptationSet(adaptationSetContent, baseUrl, duration, type) {
    ...
    const representations = adaptationSetContent.match(/<Representation[^>]*>/gi) || [];
    let bestScore = 0;
    ...
    console.log(`[IntegratedStreamProcessor] Best ${type} quality:`, selectedQuality);
    ...
    if (templateMatch) {
      ...
      if (mediaMatch && timescaleMatch && durationSegMatch) {
        ...
        if (isVideo && selectedQuality && selectedQuality.height) {
          mediaTemplate = mediaTemplate.replace(/\/video\/\d+\//, `/video/${selectedQuality.height}/`);
          if (initTemplate) {
            initTemplate = initTemplate.replace(/\/video\/\d+\//, `/video/${selectedQuality.height}/`);
          }
        }
        ...
        console.log(`[IntegratedStreamProcessor] ${type}: ${segmentCount} segments, timescale: ${timescale}`);
  ...
  async downloadSeparateStreams(streams, type, baseUrl) {
    console.log('[IntegratedStreamProcessor] Downloading separate video and audio streams...');
    ...
    try {
      ...
      this.sendProgressUpdate(0, 'Starting video and audio download...', `${videoSegments.length + audioSegments.length} segments`, '0 B', videoUrl);
      
      // Download video with progress tracking
      console.log(`[IntegratedStreamProcessor] Downloading video (${videoSegments.length} segments)...`);
      const videoBlob = await this.downloadSegments(videoSegments, 'video', videoUrl, (progress, downloadedBytes) => {
      ...
      console.log(`[IntegratedStreamProcessor] Downloading audio (${audioSegments.length} segments)...`);
      const audioBlob = await this.downloadSegments(audioSegments, 'audio', videoUrl, (progress, downloadedBytes) => {
        ...
        this.sendProgressUpdate(
          ...
          this.formatBytes(videoBlob.size + downloadedBytes),
          videoUrl
        );
      });
      
      const totalSize = videoBlob.size + audioBlob.size;
      this.sendProgressUpdate(95, '💾 Saving to Downloads folder...', 'Video + Audio', this.formatBytes(totalSize), videoUrl);
      ...
      console.log(`[IntegratedStreamProcessor] Generated filenames:`, { videoFilename, audioFilename });
      ...
      this.sendProgressUpdate(100, '✅ Hoàn thành!', 'Video + Audio lưu Downloads folder', this.formatBytes(totalDownloaded), videoUrl);
      ...
      console.log(`[IntegratedStreamProcessor] Downloaded separate files to default download folder:`, {
        video: videoFilename,
        audio: audioFilename,
      ...
      console.error('[IntegratedStreamProcessor] Separate streams download failed:', error);
      throw error;
    }
  }

  async downloadSegments(segments, streamType, videoUrl = null, progressCallback = null) {
    console.log(`[IntegratedStreamProcessor] Downloading ${segments.length} ${streamType} segments...`);
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (videoUrl) {
        ...
        console.log(`[IntegratedStreamProcessor] ${streamType} segment ${i + 1}/${segments.length}, isPaused=${state?.isPaused}`);
        
        if (state && state.isPaused) {
          console.log(`🛑 PAUSE DETECTED in ${streamType} download - waiting...`);
        ...
        if (state && state.isCanceled) {
          console.log(`[IntegratedStreamProcessor] ${streamType} download canceled at segment ${i + 1}`);
      ...
      if (this.isDownloadCanceled) {
        console.log(`[IntegratedStreamProcessor] Download canceled at ${streamType} segment ${i + 1}`);
        throw new Error('Download canceled by user');
      }
      
      try {
        const segmentUrl = segments[i];
        console.log(`[IntegratedStreamProcessor] Downloading ${streamType} segment ${i + 1}/${segments.length}: ${segmentUrl}`);
        ...
        if (!response.ok) {
          console.warn(`[IntegratedStreamProcessor] Segment ${i + 1} failed: ${response.status}`);
          continue;
        }
        ...
        console.log(`[IntegratedStreamProcessor] Downloaded ${streamType} segment ${i + 1}, size: ${arrayBuffer.byteLength}, total: ${totalSize}`);
        
      } catch (error) {
        // Check if error is abort error
        if (error.name === 'AbortError') {
          console.log(`[IntegratedStreamProcessor] 🛑 Segment ${i + 1} download aborted (paused)`);
        ...
        console.warn(`[IntegratedStreamProcessor] Error downloading ${streamType} segment ${i + 1}:`, error);
      }
    }
    ...
    const mergedBlob = new Blob(chunks, { type: streamType === 'video' ? 'video/mp4' : 'audio/mp4' });
    console.log(`[IntegratedStreamProcessor] ${streamType} merged: ${mergedBlob.size} bytes`);
    
    return mergedBlob;
  }

  async downloadAndMergeSegments(segments, type, baseUrl) {
    console.log(`[IntegratedStreamProcessor] Starting download and merge of ${segments.length} segments`);
    ...
    try {
      ...
      if (!state || state.completed) {
        ...
        this.sendProgressUpdate(0, '🎬 Bắt đầu tải video...', `${segments.length} segments`, '0 B', videoUrl);
      } else {
        // Resuming existing download
        console.log(`[IntegratedStreamProcessor] Resuming from segment ${state.currentSegment + 1}/${segments.length}`);
      }
      ...
      for (let i = state.currentSegment; i < segments.length; i++) {
        ...
        console.log(`[IntegratedStreamProcessor] Loop iteration ${i + 1}/${segments.length}, isPaused=${state.isPaused}, isCanceled=${state.isCanceled}`);
        ...
        if (state.isCanceled) {
          console.log(`[IntegratedStreamProcessor] Download canceled at segment ${i + 1}`);
          throw new Error('Download canceled by user');
        }
        
        // Check for pause BEFORE downloading segment
        if (state.isPaused) {
          console.log(`[IntegratedStreamProcessor] ✋ PAUSE DETECTED at segment ${i + 1}, entering pause wait...`);
          
          // Use chunks.length (completed segments) for accurate progress
          const completedSegments = state.chunks?.length || 0;
          const percent = completedSegments > 0 ? (completedSegments / segments.length) * 100 : 0;
          ...
          console.log(`[IntegratedStreamProcessor] Creating pause Promise for ${videoUrl}...`);
          await new Promise((resolve, reject) => {
            // Store callbacks in state
            state.resumeCallback = resolve;
            state.cancelCallback = reject;
            this.downloadStates.set(videoUrl, state);
            console.log(`[IntegratedStreamProcessor] Pause Promise created, waiting for resume or cancel...`);
          }).catch((err) => {
            console.log(`[IntegratedStreamProcessor] Pause Promise rejected:`, err);
            throw new Error('Download canceled during pause');
          });
          
          console.log(`[IntegratedStreamProcessor] Pause Promise resolved! Resuming...`);
          ...
          if (state.isPaused) {
            console.log(`[IntegratedStreamProcessor] WARNING: isPaused still true after resume! Retrying iteration...`);
            i--; // Retry this segment
            continue;
          }
          
          console.log(`[IntegratedStreamProcessor] Successfully resumed from segment ${i + 1}`);
        }
        
        // Download the segment
        try {
          const segmentUrl = segments[i];
          console.log(`[IntegratedStreamProcessor] Downloading segment ${i + 1}/${segments.length}: ${segmentUrl}`);
          
          const response = await fetch(segmentUrl, { signal: state.abortController?.signal });
          if (!response.ok) {
            console.warn(`[IntegratedStreamProcessor] Segment ${i + 1} failed: ${response.status}, continuing...`);
            continue;
          }
          ...
          if (state.isCanceled) {
            console.log(`[IntegratedStreamProcessor] Canceled during segment download`);
            throw new Error('Download canceled by user');
          }
          ...
          if (!state.isPaused) {
            ...
            const segmentProgress = ((i + 1) / segments.length * 100).toFixed(1);
            this.sendProgressUpdate(
              progress, 
          ...
          if (error.name === 'AbortError') {
            console.log(`[IntegratedStreamProcessor] 🛑 Segment ${i + 1} aborted (paused)`);
            ...
            await new Promise((resolve, reject) => {
              state.resumeCallback = () => {
                console.log(`▶️ Download RESUMED after abort at segment ${i + 2}`);
                resolve();
              };
              state.cancelCallback = () => {
          ...
          console.warn(`[IntegratedStreamProcessor] Error downloading segment ${i + 1}:`, error);
        }
      }
      
      console.log(`[IntegratedStreamProcessor] Downloaded ${state.chunks.length}/${segments.length} segments, total size: ${state.currentSize}`);
      this.sendProgressUpdate(95, '🔧 Đang ghép video...', `${state.chunks.length} segments`, this.formatBytes(state.currentSize), videoUrl);
      ...
      this.sendProgressUpdate(98, '💾 Tạo file tải xuống...', filename, this.formatBytes(mergedBlob.size), videoUrl);
      console.log(`[IntegratedStreamProcessor] Triggering download: ${filename}`);
      ...
      this.sendProgressUpdate(100, '✅ Video đã tải xong!', filename, this.formatBytes(mergedBlob.size), videoUrl, downloadId);
      ...
      console.error('[IntegratedStreamProcessor] Download and merge failed:', error);
      throw error;
    }
  }
  ...
  sanitizeFilename(filename) {
    return filename
      ...
      .substring(0, 100) || 'video'; // Limit length and fallback
  }
  ...
  sendProgressUpdate(percent, text, detail, size, videoUrl, downloadId = null, paused = false) {
    try {
    ...
    return new Promise((resolve) => {
      const checkDownload = async () => {
        try {
          ...
          console.warn('[IntegratedStreamProcessor] Error checking download:', error);
        }
        
        // Timeout check
        if (Date.now() - startTime > maxWaitMs) {
          console.warn('[IntegratedStreamProcessor] Timeout waiting for download path');
          resolve(null);
          return;
        }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log('[Background-Simple] Message received:', message?.action, 'from tab:', sender?.tab?.id);
  ...
  (async () => {
    try {
      ...
      if (message.action === 'pauseDownload') {
        console.log('[Background-Simple] pauseDownload requested for:', message.videoUrl);
      ...
      if (message.action === 'resumeDownload') {
        console.log('[Background-Simple] resumeDownload requested for:', message.videoUrl);
      ...
      if (message.action === 'debug') {
        ...
        if (message.command === 'listVideos') {
          ...
          console.log('[Background-Simple] Debug: detectedVideos Map size:', detectedVideos.size);
      ...
      if (message.action === 'manifestDetected') {
        console.log('[Background-Simple] manifestDetected:', message.type, message.url?.substring(0, 100));
        ...
        console.log('[Background-Simple] Video stored with key:', key);
        console.log('[Background-Simple] Video tabId:', tabId, 'type:', typeof tabId);
        console.log('[Background-Simple] Total videos now:', detectedVideos.size);
        // Update per-tab badge
        try { updateBadge(tabId); } catch (e) { console.warn('[Background-Simple] updateBadge call failed'); }
            ...
            .then(manifest => {
              ...
              if (durationMatch) {
                ...
                if (storedVideo) {
                  ...
                  console.log(`[Background-Simple] Updated video duration: ${duration}s`);
                }
              }
            })
            .catch(err => {
              console.log('[Background-Simple] Could not fetch duration:', err.message);
            });
            ...
            .then(manifest => {
              ...
              if (duration > 0) {
                ...
                if (storedVideo) {
                  storedVideo.duration = duration;
                  detectedVideos.set(key, storedVideo);
                  console.log(`[Background-Simple] Updated HLS duration: ${duration}s`);
                }
              }
            })
            .catch(err => {
              console.log('[Background-Simple] Could not fetch HLS duration:', err.message);
            });
        }
        ...
        try {
          ...
          const videoForPopup = detectedVideos.get(tabId).find(v => v.url === message.url);
          ...
          chrome.runtime.sendMessage({
            ...
            console.log('[Background-Simple] Could not notify popup (popup closed)');
          });
        } catch (e) {
      ...
      if (message.action === 'getVideos') {
        ...
        console.log('[Background-Simple] getVideos for tab:', requestedTabId, 'type:', typeof requestedTabId);
        console.log('[Background-Simple] detectedVideos Map size:', detectedVideos.size);
        ...
        detectedVideos.forEach((video, key) => {
          ...
          console.log('[Background-Simple] Video tabId:', video.tabId, 'type:', typeof video.tabId);
          console.log('[Background-Simple] Requested tabId:', requestedTabId, 'type:', typeof requestedTabId);
          console.log('[Background-Simple] Exact match?', video.tabId === requestedTabId);
          ...
          if (video.tabId === requestedTabId) {
            ...
            console.log('[Background-Simple] Added video to results:', video.url.substring(0, 50));
          }
        });
        
        console.log('[Background-Simple] All stored tabIds:', Array.from(allTabIds));
        console.log('[Background-Simple] Requested tabId:', requestedTabId);
        console.log('[Background-Simple] Returning', tabVideos.length, 'videos for tab', requestedTabId);
        
        // If no exact match found but we have videos, try to return recent videos
        if (tabVideos.length === 0 && detectedVideos.size > 0) {
          console.log('[Background-Simple] No exact tabId match, returning all recent videos');
          const recentVideos = [];
          const now = Date.now();
          detectedVideos.forEach((video, key) => {
          ...
          console.log('[Background-Simple] Returning', recentVideos.length, 'recent videos instead');
      ...
      if (message.action === 'getAllVideos') {
        ...
        console.log('[Background-Simple] Returning ALL videos:', allVideos.length);
      ...
      if (message.action === 'clearVideos') {
        ...
        console.log('[Background-Simple] Cleared', keysToDelete.length, 'videos');
      ...
      if (message.action === 'cancelDownload') {
        console.log('[Background-Simple] cancelDownload requested for:', message.videoUrl);
        streamProcessor.cancelDownload(message.videoUrl);
        sendResponse({ success: true });
        return;
      }
      ...
      if (message.action === 'downloadVideo') {
        console.log('[Background-Simple] downloadVideo requested for:', message.video?.url?.substring(0, 100));
        ...
        try {
          ...
          if (video.type === 'dash' || video.type === 'mpd' || video.type === 'hls') {
            console.log('[Background-Simple] Using IntegratedStreamProcessor for streaming content...');
            
            // Process the stream
            const result = await streamProcessor.processStream(video, message.options || {});
            
            console.log('[Background-Simple] IntegratedStreamProcessor result:', result);
            sendResponse({ 
              success: true, 
              result: result
            });
            ...
            console.log('[Background-Simple] Direct download for regular video...');
